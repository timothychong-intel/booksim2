...
SHARED int64_t * SwmSpmat::rand_permp(int64_t N, int seed) {
   SHARED int64_t * p;
...
/*!
  ...
  \ingroup spmatgrp
  */
sparsemat_t * SwmSpmat::permute_matrix(sparsemat_t *omat, SHARED int64_t *rperminv, SHARED int64_t *cperminv) {
 ...
 * \ingroup spmatgrp
 */
sparsemat_t * SwmSpmat::transpose_matrix(sparsemat_t *omat) {
   sparsemat_t * A;
 ...
 * \ingroup spmatgrp
 */
int SwmSpmat::is_lower_triangular(sparsemat_t *A, int64_t unit_diagonal) {
   int64_t i,j; // * ltri_rperm, * ltri_cperm;
   ...
   err = _lgp->lgp_reduce_add_l(err);
   err2 = (unit_diagonal ? _lgp->lgp_reduce_add_l(err2) : 0);
   if( err || err2 ){
 ...
 * \ingroup spmatgrp
 */
int SwmSpmat::is_upper_triangular(sparsemat_t *A, int64_t unit_diagonal) {
 ...
 * \ingroup spmatgrp
 */
int64_t SwmSpmat::tril(sparsemat_t * A, int64_t k) {
   ...
   int64_t new_nnz = _lgp->lgp_reduce_add_l(pos);
 ...
 * \ingroup spmatgrp
 */
int64_t SwmSpmat::triu(sparsemat_t * A, int64_t k) {
   // remove entries below the diagonal
   int64_t i, j, col, pos = 0, start = 0;
   int weighted = (A->value != NULL);
   for(i = 0; i < A->lnumrows; i++){
 ...
 * \ingroup spmatgrp
 */
int SwmSpmat::is_perm(SHARED int64_t * perm, int64_t N) {
   int64_t i;
   int64_t * lperm = lgp_local_part(int64_t, perm);
   SHARED int64_t * flag = (int64_t *) _lgp->lgp_all_alloc(N, sizeof(int64_t));
   ...
   _lgp->lgp_barrier();
   for(i = 0; i < l_N; i++)
      _lgp->lgp_put_int64(flag, lperm[i], 1);
   _lgp->lgp_barrier();
   int64_t err = 0L;
   for(i = 0; i < l_N; i++)
      if(lflag[i] == 0)
         err++;
   _lgp->lgp_all_free(flag);
   err = _lgp->lgp_reduce_add_l(err);
   return(err == 0L);
}
...
sparsemat_t * SwmSpmat::random_graph(int64_t n, graph_model model, edge_type edgetype,
      self_loops loops,
   ...
   if(model == FLAT){
      ...
      if(!MYTHREAD && 0){
         point_t pt;
         for(int i = 0; i < n; i++){
            _lgp->lgp_memget(&pt, op, sizeof(point_t), i);
            //printf("%ld: %lf %lf\n", i, pt.x, pt.y);
         }
      }
 ...
 */
sparsemat_t * SwmSpmat::erdos_renyi_random_graph(int64_t n, double p, edge_type edgetype, self_loops loops, uint64_t seed){
   ...
   row = MYTHREAD;
   col = 1 + floor(log(1 - _lgp->lgp_rand_double()) / D);
   while(row < n){
      if(edgetype == UNDIRECTED || edgetype == UNDIRECTED_WEIGHTED)
         end = row;
      while(col < end){
   ...
   if(weighted){
      for(i = 0; i < lnnz; i++){
         A->lvalue[i] = _lgp->lgp_rand_double();
      }
   }
 ...
 * \return A distributed sparsemat_t that holds the graph.  It is either a lower triangular  matrix
 or square matrix, weighted or not with or without a diagonal.
 */
sparsemat_t * SwmSpmat::erdos_renyi_random_graph_naive(int64_t n, double p, edge_type edgetype, self_loops loops, uint64_t seed){
...
sparsemat_t * SwmSpmat::generate_kronecker_graph_from_spec(int mode, int * spec, int num){
   int i;
   ...
   int64_t *B_spec = (int64_t *) calloc(num, sizeof(int64_t));
   int64_t *C_spec = (int64_t *) calloc(num, sizeof(int64_t));
   int64_t B_num = num/2;
 ...
 * \ingroup spmatgrp
 */
sparsemat_t * SwmSpmat::kronecker_product_graph_dist(sparsemat_t * B, sparsemat_t * C) {

   int64_t lrow, row, rowB, rowC, col, pos, j, k; //, i
   ...
   _lgp->lgp_barrier();
   A->lnnz = pos;
   A->nnz = _lgp->lgp_reduce_add_l(pos);

#if 0
   SHARED int64_t * sh_data = _lgp->lgp_all_alloc(THREADS, sizeof(int64_t));
   int64_t * lsh_data = _lgp->lgp_local_part(int64_t, sh_data);
   lsh_data[0] = 0;
   _lgp->lgp_barrier();
   ...
   if(!MYTHREAD){
      ...
      if(A->lnonzero[A->loffset[0]] == 0 && A->loffset[1] != A->loffset[0]){
         ...
         A->lnnz--;
         for(i = 0; i < THREADS; i++)
            _lgp->lgp_atomic_add(sh_data, i, 1);
      }
   }
 ...
 * \ingroup spmatgrp
 */
sparsemat_t * SwmSpmat::kronecker_product_graph_local(sparsemat_t * B, sparsemat_t * C) {
   ...
   int64_t * tmp_offset = (int64_t *) calloc(n + 1, sizeof(int64_t));
 ...
 * \ingroup spmatgrp
 */
sparsemat_t * SwmSpmat::gen_star_graph(int64_t m, int mode) {
 ...
 * \ingroup spmatgrp
 */
sparsemat_t * SwmSpmat::kronecker_product_of_stars(int64_t M, int64_t * m, int mode) {
   int64_t i;
   ...
   sparsemat_t ** Aarr = (sparsemat_t **) calloc(2*M, sizeof(sparsemat_t *));
   for(i = 0; i < M; i++)
 ...
 * \ingroup spmatgrp
 */
int SwmSpmat::sort_nonzeros( sparsemat_t *mat) {
   int i,j;
   if(mat->value){
      ...
      col_val_t * tmparr = (col_val_t *) calloc(max, sizeof(col_val_t));
...
int64_t SwmSpmat::calc_num_tri_kron_graph(int kron_mode, int * kron_spec, int kron_num)
{
   int i;
 ...
 * \ingroup spmatgrp
 */
int SwmSpmat::compare_matrix(sparsemat_t *lmat, sparsemat_t *rmat) {
   int i,j;
 ...
 */
sparsemat_t * SwmSpmat::direct_undirected_graph(sparsemat_t * L){
   sparsemat_t * A = NULL;
   return (A);
}
...
int64_t * lrowcounts = (int64_t *) calloc(A->lnumrows, sizeof(int64_t));
for(i = 0; i < el->num; i++){
   if(weighted)
 ...
 */
sparsemat_t * SwmSpmat::copy_matrix(sparsemat_t *srcmat) {
   int i,j;
   //int64_t numrows, numcols, lnumrows;
 ...
 */
int SwmSpmat::spmat_are_eq_doubles(double a, double b) {
   if(a == 0.0){
 ...
 * \ingroup spmatgrp
 */
sparsemat_t * SwmSpmat::init_matrix(int64_t numrows, int64_t numcols, int64_t nnz_this_thread, int weighted)
{
   ...
   mat->numcols  = numcols;
   mat->offset   = (int64_t *) _lgp->lgp_all_alloc(mat->numrows + THREADS, sizeof(int64_t));
   if(mat->offset == NULL){
   ...
   int64_t max = _lgp->lgp_reduce_max_l(nnz_this_thread);

   _lgp->lgp_barrier();

   //if(!MYTHREAD)
   //printf("[%d] init_matrix nnz %ld max %ld\n", MYTHREAD, nnz_this_thread, max*THREADS);

   int64_t total = _lgp->lgp_reduce_add_l(nnz_this_thread);

   mat->nonzero = (int64_t *) _lgp->lgp_all_alloc(max*THREADS, sizeof(int64_t));
   if(mat->nonzero == NULL){
   ...
   if(weighted){
      mat->value = (double *) _lgp->lgp_all_alloc(max*THREADS, sizeof(double));
      if(mat->value == NULL){
 ...
 * \ingroup spmatgrp
 */
sparsemat_t * SwmSpmat::init_local_matrix(int64_t numrows, int64_t numcols, int64_t nnz) {
   sparsemat_t * mat = (sparsemat_t *) calloc(1, sizeof(sparsemat_t));
   mat->local = 1;
   mat->numrows  = numrows;
   ...
   mat->numcols  = numcols;
   mat->offset   = NULL;
   mat->loffset  = (int64_t *) calloc(mat->lnumrows + 1, sizeof(int64_t));
   if(mat->loffset == NULL){
      clear_matrix(mat);
      free(mat);
      return(NULL);
   ...
   mat->nonzero = NULL;
   mat->lnonzero = (int64_t *) calloc(nnz, sizeof(int64_t));
 ...
 * \ingroup spmatgrp
 */
edge_list_t * SwmSpmat::init_edge_list(int64_t nalloc, int weighted){
   edge_list_t * el = (edge_list_t *) calloc(1, sizeof(edge_list_t));
   el->num = 0;
   el->nalloc = nalloc;
   if(!weighted){
      el->edges = (edge_t *) calloc(nalloc, sizeof(edge_t));
      el->wedges = NULL;
   }else{
      el->edges = NULL;
      el->wedges = (w_edge_t *) calloc(nalloc, sizeof(w_edge_t));
   }
   return(el);
}
 ...
 * \ingroup spmatgrp
 */
void SwmSpmat::clear_edge_list(edge_list_t * el){
   free(el->edges);
   free(el);
}
 ...
 * \ingroup spmatgrp
 */
triples_t * SwmSpmat::init_triples(int64_t numrows, int64_t numcols, int64_t nalloc, int weighted){
   triples_t * T = (triples_t *) calloc(1, sizeof(triples_t));
   T->numrows = numrows;
   T->numcols = numcols;
   T->lnnz = 0;
   T->nalloc = nalloc;
   T->row = (int64_t *) calloc(T->nalloc, sizeof(int64_t));
   T->col = (int64_t *) calloc(T->nalloc, sizeof(int64_t));
   if(weighted) T->val = (double *) calloc(T->nalloc, sizeof(double));
   else T->val = NULL;
   return(T);
}
 ...
 * \ingroup spmatgrp
 */
void SwmSpmat::clear_matrix(sparsemat_t * mat) {
   if(mat->local){
      free(mat->lnonzero);
      free(mat->loffset);
   }else{
      _lgp->lgp_all_free(mat->nonzero);
      _lgp->lgp_all_free(mat->offset);
   }
}
 ...
 * \ingroup spmatgrp
 */
void SwmSpmat::clear_triples(triples_t * T) {
   free(T->val);
   free(T->row);
   free(T->col);
}
 ...
 * \ingroup spmatgrp
 */
void SwmSpmat::clear_spmat_dataset(spmat_dataset_t * spd){
   free(spd->dirname);
 ...
 */
int64_t SwmSpmat::append_triple(triples_t * T, int64_t row, int64_t col, double val){
   if(T->nalloc == T->lnnz){
      ...
      T->row = (int64_t *) realloc(T->row, T->nalloc*sizeof(int64_t));
      T->col = (int64_t *) realloc(T->col, T->nalloc*sizeof(int64_t));
      if(T->val)T->val = (double *) realloc(T->val, T->nalloc*sizeof(double));
   }
   T->row[T->lnnz] = row;
   T->col[T->lnnz] = col;
 ...
 */
int64_t SwmSpmat::append_edge(edge_list_t * el, int64_t row, int64_t col){
   if(el->nalloc == el->num){
      ...
      el->edges = (edge_t *) realloc(el->edges, el->nalloc*sizeof(edge_t));
   }
 ...
 * \return the number of edges after this edge was appended.
 */
int64_t SwmSpmat::append_weighted_edge(edge_list_t * el, int64_t row, int64_t col, double val){
   if(el->nalloc == el->num){
      //printf("PE %d: out of space! nalloc = %ld\n", MYTHREAD, el->nalloc);
      // we need to expand our allocations!
      if(el->nalloc < 10000)
         ...
         el->nalloc = el->nalloc*1.25;
      //printf("PE %d: new space! nalloc = %ld\n", MYTHREAD, el->nalloc);
      el->wedges = (w_edge_t *) realloc(el->wedges, el->nalloc*sizeof(w_edge_t));
   }
 ...
 */
sparsemat_t * SwmSpmat::triples_to_sparsemat(triples_t * T){

   int64_t i, j;
   // get rowcounts (to create tmp offset array)
   int64_t * tmp = (int64_t *) calloc(T->lnumrows + 1, sizeof(int64_t));
   for(i = 0; i < T->lnnz; i++){
      tmp[T->row[i] + 1]++;
   }
   ...
   if(!A){
       free(tmp);
       printf("ERROR: triples_to_sparsemat: init failed.\n"); return(NULL);
   }
 ...
 */
void SwmSpmat::print_matrix(sparsemat_t * A){
   int64_t i, j;

   for(i = 0; i < A->numrows; i++){
      T0_printf( "row %ld: ",i);
      int64_t start = _lgp->lgp_get_int64(A->offset, i);
      int64_t end = _lgp->lgp_get_int64(A->offset, i+THREADS);
      for(j = start; j < end; j++){
         if( A->value != NULL ){
            T0_printf( "(%ld, %lg) ", _lgp->lgp_get_int64(A->nonzero, j*THREADS + i%THREADS),
                  _lgp->lgp_get_double(A->value, j*THREADS + i%THREADS));
         } else {
            T0_printf( "%ld ", _lgp->lgp_get_int64(A->nonzero, j*THREADS + i%THREADS));
         }
      }
      T0_printf( "\n");
   }
 ...
 */
int64_t SwmSpmat::rowcount_l( sparsemat_t *mat, int64_t l_row ) {
 ...
 * \return the number of nonzero in the request row
 */
int64_t SwmSpmat::rowcount_S( sparsemat_t *mat, int64_t S_row ) {
 ...
 */
nxnz_t * SwmSpmat::init_nxnz( sparsemat_t *mat ) {
   nxnz_t *ret = (nxnz_t *) calloc(1,sizeof(nxnz_t));
   ret->mat = mat;
   ret->row = -1;
 ...
 * \return void
 */
void SwmSpmat::first_l_nxnz( nxnz_t *nxz, int64_t l_row ) {
 ...
 */
bool SwmSpmat::has_l_nxnz( nxnz_t *nxz, int64_t l_row ) {
 ...
 * \return void
 */
void SwmSpmat::incr_l_nxnz( nxnz_t *nxz, int64_t l_row ) {
   assert( l_row == nxz->row );
   nxz->idx += 1;
   nxz->col   = nxz->mat->lnonzero[nxz->idx];
}
 ...
 * \return void
 */
void SwmSpmat::first_S_nxnz( nxnz_t *nxz, int64_t S_row ) {
   assert( S_row < nxz->mat->numrows );
   nxz->row   = S_row;
   nxz->first = _lgp->lgp_get_int64(nxz->mat->offset, S_row);
   nxz->idx   = nxz->first;
   nxz->stop  = _lgp->lgp_get_int64(nxz->mat->offset, S_row+THREADS);
   nxz->col   = _lgp->lgp_get_int64(nxz->mat->nonzero, (nxz->idx) * THREADS +  (nxz->row)%THREADS );
}
 ...
 * \return true if we are at a valid index, false otherwise
 */
bool SwmSpmat::has_S_nxnz( nxnz_t *nxz, int64_t S_row ) {
 ...
 * \return void
 */
void SwmSpmat::incr_S_nxnz( nxnz_t *nxz, int64_t S_row ) {
   assert( S_row == nxz->row );
   nxz->idx += 1;
   nxz->col   = _lgp->lgp_get_int64(nxz->mat->nonzero, (nxz->idx) * THREADS +  (nxz->row)%THREADS );
}
...
int SwmSpmat::nz_comp(const void *a, const void *b) {
   return( *(uint64_t *)a - *(uint64_t *)b );
}

/*! \brief comparison function to support qsort */
int SwmSpmat::point_comp(const void *a, const void *b) {
...
int SwmSpmat::col_val_comp(const void *a, const void *b) {
 ...
 */
int SwmSpmat::edge_comp(const void *a, const void *b)
{
 ...
 * We sort on the columns so that the matrix will be "tidy"
 */
int SwmSpmat::w_edge_comp(const void *a, const void *b)
{
 ...
 * \ingroup spmatgrp
 */
d_array_t * SwmSpmat::init_d_array(int64_t num)
{
   d_array_t * array = (d_array_t *) calloc(1, sizeof(d_array_t));
   array->num  = num;
   array->lnum = (num + THREADS - MYTHREAD - 1)/THREADS;
   array->entry   = (double *) _lgp->lgp_all_alloc(num + THREADS, sizeof(double));
   if(array->entry == NULL){
 ...
 * \ingroup spmatgrp
 */
void SwmSpmat::set_d_array(d_array_t * A, double v)
{
   int64_t i;
   for(i=0; i<A->lnum; i++) {
      A->lentry[i] = v;
 ...
 */
d_array_t * SwmSpmat::copy_d_array(d_array_t * S)
{
   int64_t i;
   _lgp->lgp_barrier();
 ...
 */
int64_t SwmSpmat::replace_d_array(d_array_t * D, d_array_t * S)
{
 ...
 * \param A the d_array
 */
void SwmSpmat::clear_d_array(d_array_t * A)
{
   _lgp->lgp_all_free(A->entry);
}
