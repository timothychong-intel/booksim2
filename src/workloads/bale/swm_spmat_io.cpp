...
spmat_dataset_t * SwmSpmat::read_sparse_matrix_metadata(char * dirname){

   spmat_dataset_t * spd = (spmat_dataset_t *) calloc(1, sizeof(spmat_dataset_t));

   spd->dirname = (char *) calloc(strlen(dirname)+1, sizeof(char));
   ...
   spd->numrows = _lgp->lgp_reduce_add_l(spd->numrows);
   spd->numcols = _lgp->lgp_reduce_add_l(spd->numcols);
   spd->nnz     = _lgp->lgp_reduce_add_l(spd->nnz);
   spd->nwriters= _lgp->lgp_reduce_add_l(spd->nwriters);
   spd->values  = _lgp->lgp_reduce_add_l(spd->values);
...
int SwmSpmat::write_sparse_matrix_metadata(spmat_dataset_t * spd){
   if(MYTHREAD == 0){
 ...
 * \ingroup spmatgrp
 */
spmat_dataset_t * SwmSpmat::open_sparse_matrix_write(char * dirname, sparsemat_t * A){
   ...
   int ret = mkdir(dirname, 0770);
   assert(!ret);

   /* write metadata file */
   ret = write_sparse_matrix_metadata(spd);
   assert(ret == 0);
 ...
 * \ingroup spmatgrp
 */
spmat_dataset_t * SwmSpmat::open_sparse_matrix_read(char * dirname, int64_t nreaders){
   int64_t i;
   ...
   spd->global_first_row_to_me = (int64_t *) calloc(THREADS, sizeof(int64_t));
 ...
 */
void SwmSpmat::read_row_info(spmat_dataset_t * spd){
   ...
   if(spd->io_rank >= 0){
      ...
      spd->nrows_in_file      = (int64_t *) calloc(spd->nfiles, sizeof(int64_t));
      spd->nrows_read_in_file = (int64_t *) calloc(spd->nfiles, sizeof(int64_t));
      spd->rowcnt             = (int64_t *) calloc(spd->lnumrows, sizeof(int64_t));
   ...
   for(i = 0; i < spd->nfiles; i++){
      ...
      if(i == 0 && spd->start_row_first_file){
         ...
         int ret = fseek(fp, spd->start_row_first_file*sizeof(int64_t), SEEK_SET);
         assert(!ret);
      }
...
void SwmSpmat::write_row_info(spmat_dataset_t * spd, sparsemat_t * A){

   int64_t i;

   /* get the rowcounts of the matrix into a distributed array */
   SHARED int64_t * rowcnt = (int64_t *) _lgp->lgp_all_alloc(A->numrows, sizeof(int64_t));
   ...
   if(spd->lnumrows){
      ...
      for(i = 0; i < spd->lnumrows; i++){
         ...
         int64_t rc = _lgp->lgp_get_int64(rowcnt, spd->global_first_row + i);
         offset += rc;
   ...
   error = _lgp->lgp_reduce_add_l(error);
   assert(error == 0);
 ...
 */
int64_t SwmSpmat::read_nonzeros_buffer(spmat_dataset_t * spd, int64_t * buf,
   ...
   if(spd->current_file >=  spd->first_file + (long signed) spd->nfiles)
      return(-1);
   ...
   if(spd->file_open == 0){
      ...
      if(spd->current_file == spd->first_file){
         ...
         int ret = fseek(spd->nnzfp, spd->first_file_offset*sizeof(int64_t), SEEK_SET);
         assert(!ret);
         //fprintf(stderr,"PE %d: seeking to %ld in nonzero file %ld\n", MYTHREAD, spd->first_file_offset, spd->current_file);
         if(spd->values){
             ret = fseek(spd->valfp, spd->first_file_offset*sizeof(double), SEEK_SET);
             assert(!ret);
         }
      }
   }
 ...
 * \ingroup spmatgrp
 */
int SwmSpmat::write_matrix_mm(sparsemat_t *A, char * name) {
   if(!MYTHREAD){
 ...
 * \ingroup spmatgrp
 */
sparsemat_t * SwmSpmat::read_matrix_mm_to_dist(char * name) {
   ...
   sh_data = (int64_t *) _lgp->lgp_all_alloc (THREADS*4, sizeof(int64_t));
   ...
   if(!MYTHREAD){
      int fscanfret;
      int64_t * nnz_per_th = (int64_t *) calloc(THREADS, sizeof(int64_t));
      ...
      // Changing this to not use fscanf (coverity doesn't like it)
      ...
      string line(buffer);
      assert(line.rfind("%%MatrixMarket", 0) == 0);
      size_t mypos = line.find(' ');
      assert(mypos != std::string::npos);
      line = line.substr(mypos + 1);
      std::istringstream iss(line);
      if (iss >> object >> format >> field){
        fscanfret = 3;
      }
      ...
      assert(nr < MAX_CALLOC_SIZE && nr >= 0);
      // allocate space to store the matrix data
      rowcount = (int64_t *) calloc(nr, sizeof(int64_t));
      if(!rowcount){
         T0_printf( "ERROR: read_matrix_mm_to_dist: could not allocate arrays\n");
         for(i = 0; i < THREADS; i++) _lgp->lgp_put_int64(sh_data, i, -1);
      }
      assert(rowcount != NULL);  //For coverity static analysis

      // read the data
      int64_t row, col, val, pos = 0;
     assert(nnz < MAX_CALLOC_SIZE && nnz >= 0);
      if(values == 0){
         edges = (edge_t *) calloc(nnz, sizeof(edge_t));
         ...
         tri = (w_edge_t *) calloc(nnz, sizeof(w_edge_t));
      ...
      for(i = 0; i < THREADS; i++){
         _lgp->lgp_put_int64(sh_data, i, nnz_per_th[i]);
         _lgp->lgp_put_int64(sh_data, i+THREADS, nr);
         _lgp->lgp_put_int64(sh_data, i+2*THREADS, nc);
         _lgp->lgp_put_int64(sh_data, i+3*THREADS, values);
      }
      free(rowcount);
      free(nnz_per_th);
   ...
   SHARED int64_t * tmp_offset = (int64_t *) _lgp->lgp_all_alloc(nr + THREADS, sizeof(int64_t));
   if(!A || !tmp_offset){
      T0_printf( "ERROR: read_matrix_mm_to_dist: failed to init matrix or tmp_offset!\n");
      _lgp->lgp_all_free(tmp_offset);
      free(A);
      return(NULL);
   }
   ...
   _lgp->lgp_barrier();
   _lgp->lgp_all_free(sh_data);

   if(!MYTHREAD){
      for(i = 0; i < nr; i++)
         _lgp->lgp_put_int64(tmp_offset, i, rowcount[i]);
      free(rowcount);
   }
