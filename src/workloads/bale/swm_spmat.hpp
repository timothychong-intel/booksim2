...
#define spmat_INCLUDED  /*!< std trick */
...
//Coverity static analysis requires that we do a sanity check
#define MAX_CALLOC_SIZE 1e9
...
class SwmSpmat
{
   ...
   void init_spmat(SwmLibgetput * lgp, int me, int np) { _lgp = lgp;  MYTHREAD = me; THREADS = np; }

   //SwmShmem * getShmem() { return _so; }
   //int shmem_n_pes() { return _so->shmem_n_pes(); }
   ...
   sparsemat_t *       geometric_random_graph(int64_t n, double r, edge_type edgetype, self_loops loops, uint64_t seed, SHARED point_t ** out_points) { return NULL; }
   ...
   sparsemat_t *       permute_matrix_agp_nbi(sparsemat_t * A, SHARED int64_t * rperminv, SHARED int64_t * cperminv);
   ...
   SHARED int64_t *    build_histogram(sparsemat_t * A);
   ...
   sparsemat_t *       transpose_matrix_agp_nbi(sparsemat_t * A);
   ...
   static int nz_comp(const void *a, const void *b);
   int point_comp(const void *a, const void *b);
   static int col_val_comp(const void *a, const void *b);
   //int dbl_comp(const void *a, const void *b);
   static int edge_comp(const void *a, const void *b);
   static int w_edge_comp(const void *a, const void *b);
